# -*- coding: utf-8 -*-
"""
Created on Wed Oct 10 09:56:07 2018
...
data = pickler.loadPickle(ProcData_location,"GAN6_24_10_2018")   
x= data['x']
y= data['y']
...
def Generator(data,z):
    ...
    #Unidirectional stacked LSTM CELLs
    generator_LSTM = LSTM(cell_size,return_sequences=True,return_state=True)
    ...
    generator_outputs_final,_,_ = generator_LSTM3(gen_outputs)
    
    #dense layers
    #generator_dense = Dense(hidden_units, activation = 'linear')(generator_outputs_final)
    generator_dense2 = Dense(realshape[-1], activation = 'relu',name='Output_G')(generator_outputs_final)   
...
def Discriminator(data,labels):
    ...
    discriminator_LSTM3 = LSTM(cell_size,return_sequences=True,return_state=True)
    
    dis_outputs, _, _ = discriminator_LSTM(discriminator_inputs)
    #dis_outputs2, _, _ = discriminator_LSTM2(dis_outputs)    
    dis_outputs_final,_,_ = discriminator_LSTM3(dis_outputs)
    
    #last dense to output a probability
    discriminator_dense2 = Dense(1,activation='linear',kernel_initializer='normal',name="Output_D")(dis_outputs_final)
    #discriminator_outputs_final3 = discriminator_dense2(dis_outputs_final)
    
    model= Model(inputs=discriminator_inputs, outputs=discriminator_dense2, name="D")
    print(model.summary())
    return model
...
def generate_songs(generator_model, epoch):
    ...
    test_song_stack = generator_model.predict(np.random.normal(size=(5, 300,hidden_units)))
    test_song_stack = np.squeeze(test_song_stack.astype(np.uint8))
    return test_song_stack
    

"""-----remember to check the cleanup samples code------"""
...
generator_model.compile(optimizer=RMSprop(lr=0.0001,clipnorm=clip_norm), loss=d_loss)
...
discriminator_model.compile(optimizer=RMSprop(lr=0.0001,clipnorm=clip_norm),
                            loss=[d_loss,d_loss])    
...
positive_y = np.ones((batch_size, 1,1), dtype=np.float32)
...
for epoch in range(200):
    ...
    print("discriminator_overall_loss : %s"%discriminator_overall_loss[-1])
    print("generator_overall_loss : %s"%generator_overall_loss[-1])


f=GeneratedSongsPerEpoch[99]

ppp=f[0]
reals=[]
fakes=[]
idk=[]
for p in discriminator_overall_loss:
    reals.append(p[0])
    fakes.append(p[1])
    idk.append(-p[2]/16.8)

gen2=[abs(k)/45 for k in generator_overall_loss]


#############################################################################################
pp=G_z.predict(np.random.normal(size=(10,300,hidden_units)))
